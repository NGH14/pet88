...
import { Button, Drawer, Space, Tag, Form, Input, Select, Popconfirm, Upload, Modal } from 'antd';
...
export default function TableHotel() {
...

    {
      ...
      render: (services) =>
        services.map((service) => (
          <Tag color={service?.length > 5 ? 'cyan' : 'gold'}>{t(service)}</Tag>
        ))
    },
    {
      width: 110,
      ...
      render: (_, record) => (
        <Space size="middle">
          ...
          <Button
            ...
            key="update"
            icon={<EditOutlined />}
            onClick={() => handleOpenUpdateHotel(record)}
          ></Button>
        </Space>
      )
    }
  ...
  const handleDeleteMultipleHotel = async () => {
    try {
      ...
      setListHotels(listHotels.filter((item) => !selectedRowKeys.includes(item._id)));
  ...
  return (
    <>
      <section className="tablehotel-header">
        <section className="tablehotel_leftheader">
          <h2 className="tablehotel-header-title">{t('management deparment')}</h2>
          <Button
        ...
        <section className="tablehotel_rightheader">
          <SearchTableInput
            columns={columns}
      ...
      >
        <section>
          <h6 style={{ fontWeight: 700, fontSize: 16 }}>
            {t('Delete')} {selectedRowKeys.length} {t('department')} {t('selected')}?
          </h6>
