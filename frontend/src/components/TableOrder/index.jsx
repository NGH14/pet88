...
export default function TableOrder() {
		...
		return (
			<>
				{record.service === 'hotel' ? (
					<Table columns={subColumns} dataSource={record.products} pagination={false} />
				) : null}
				<p
					style={{
						margin: 15,
	...
	return (
		<>
			<section className="tableorder-header">
				<section className="tableorder_leftheader">
					<h2 className="tableorder-header-title">{t('management order')}</h2>
					<Button
				...
				<section className="tableorder_rightheader">
					<SearchTableInput
						columns={columns}
				...
				{openUpdate ? (
					...
					>
						<Form.Item name="confirm" label={t('Confirm')}>
							<Select>
								<Option value="unconfimred">{t('Unconfimred')}</Option>
								<Option value="confimred">{t('Confimred')}</Option>
							</Select>
						</Form.Item>
